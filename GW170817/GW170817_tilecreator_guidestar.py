...
print("first block is", len(filtered_data))

filtered_ra = filtered_data['RA']
filtered_dec = filtered_data['DEC']


coordinates1 = SkyCoord([(float(x))*u.deg for x in (filtered_ra.tolist())],[float(x)*u.deg for x in filtered_dec.tolist()])#,[float(x) for x in df_result[2][1:].tolist()])
...
searched_prob170817 = result1.searched_prob[result1.searched_prob < 0.5]

print(len(filtered_data))
cutdown = filtered_data[result1.searched_prob < 0.5]
print(len(cutdown))

center = [196.8906399999999906,-24.0086059999999968] #NGC4993 coords

radius_redcircle = 4.0
cutdown['PROB'] = 1-searched_prob170817
cutdown['LOCAL'] = ((np.cos(cutdown["DEC"]*np.pi/180)*(cutdown["RA"]-center[0]))**2 + (cutdown["DEC"]-center[1])**2 < radius_redcircle**2)
...
""" #has a scale of 0.3 and red circle radius of 3.5, has total prob of 334.906963743569
centers_170817 = [(-162.8501923788646+360, -23.55),
 (-162.59038475772934+360, -24.0),
 (-162.85019237886468+360, -24.45),
 (-162.33057713659403+360, -24.45),
 (-163.36980762113535+360, -23.55),
 (-163.36980762113535+360, -24.45),
 (-163.6296152422707+360, -24.0)] """
...
for circlecenter in zip(centers_170817, label):
    ...
    for table_name in result.keys():
        ...
        #print(query_result)
    query_result['LOCAL'] = ((np.cos(centers_170817[0][1]/(180/np.pi))*(query_result["RAJ2000"]-centers_170817[0][0]))**2 + (query_result["DEJ2000"]-centers_170817[0][1])**2 < radius_tile**2)
