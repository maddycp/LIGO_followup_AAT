...
def get_field_ET(file):

	# Read in the list of tile centres and return the centre
	centres = []
	with open(f"./GW170817/GW170817_centres.txt", 'r') as f:
		lines = f.readlines()
		for ln in lines:
			lnsplit = re.split(r"[(,)]", ln)
			centres.append((float(lnsplit[1]), float(lnsplit[2])))
	centres = pd.DataFrame(centres, columns =['RA', 'DEC'])

	targets = pd.read_csv(file, delim_whitespace=True)
	targets["8"] = np.floor(8 - (targets['9'].rank(numeric_only=True, method='first') / (len(targets)/4))).astype(int)

	# Return dataframe containing targets
	return centres.loc[0], targets


# Reads in the guide and sky fibre files, gives appropriate tag and priorities
def get_guides_sky(field, tile):

	# Read in the guide star file and isolate those in the localisation region
	guides = pd.read_csv( f"./{field}/StarsRetracted/{field}_tile{tile}_stars_retract.txt", delim_whitespace=True)
...
def write_fld(obs_date, centre, targets, guides, sky):

	guide_min, guide_max = guides["Vmag"].min(), guides["Vmag"].max()

	header = (
		f"* Primary Target: GW170817\n"
		f"*\n"
		...
		f"LABEL Primary target GW170817_ET\n"
		f"UTDATE {obs_date}\n"
	...
	for i, row in targets.iterrows():
		strings.append(" ".join(row.to_numpy().astype(str)))
	...
	with open(f"./GW170817/GW170817_ET.fld", "w") as out:
...
if __name__ == "__main__":
	...
	# Read in the extra targets file and create a fld output
	centre, targets = get_field_ET("./GW170817/GW170817_ET.txt")
	guides, sky = get_guides_sky("GW170817", 0)
	write_fld(obs_date, centre, targets, guides, sky)
